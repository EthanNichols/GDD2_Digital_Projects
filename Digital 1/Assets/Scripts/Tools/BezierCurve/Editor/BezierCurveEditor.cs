...
public class BezierCurveEditor : Editor
{
		...
		void OnEnable()
		{
				...
				closeProp = serializedObject.FindProperty("close");
		...
		void DrawPointInspector(BezierPoint point, int index)
		{
				...
				SerializedProperty handle1Prop = serObj.FindProperty("handle1");
				SerializedProperty handle2Prop = serObj.FindProperty("handle2");
		...
		{
				foreach (BezierPoint p in curve.Points)
				{
						if (p != caller)
						{
								DrawPointSceneGUI(p);
